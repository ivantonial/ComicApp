...
public class CDCharacter: NSManagedObject {
    ...
    @NSManaged public var lastUpdated: Date?
    @NSManaged public var cachedAt: Date?
    @NSManaged public var favoritedAt: Date?  // Adicionado para rastrear quando foi favoritado
    ...
    public func update(from character: ComicVineAPI.Character,
        ...
        // altera o favorito se um valor foi passado (preserva estado atual se nil)
        if let isFavorite {
            let wasAlreadyFavorite = self.isFavorite
            self.isFavorite = isFavorite

            // Se acabou de ser favoritado, registra a data
            if isFavorite && !wasAlreadyFavorite {
                self.favoritedAt = now
            } else if !isFavorite && wasAlreadyFavorite {
                // Se foi removido dos favoritos, limpa a data
    ...
    /// Usa o metodo helper `makeFromCache` para criar um Character basico
    public func toCharacter() -> ComicVineAPI.Character? {
        // Verificar se temos os dados mínimos necessarios
        guard let name = self.name else {
...
extension CDCharacter {
    ...
    static func entityDescription() -> NSEntityDescription {
        ...
        e.properties = [
            ...
            CoreDataStack.cdMakeAttribute(name: "lastUpdated", type: .dateAttributeType),
            CoreDataStack.cdMakeAttribute(name: "cachedAt", type: .dateAttributeType),
            CoreDataStack.cdMakeAttribute(name: "favoritedAt", type: .dateAttributeType)  // Adicionado
        ]
        return e
    }
}
